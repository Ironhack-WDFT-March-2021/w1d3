...
function findLongestWord(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    // console.log(array[i])
    // is the word we are at longer than the currently longest word
    if (array[i].length > longestWord.length) {
      // the word we are at becomes now the currently longest word
      longestWord = array[i]
    }
  }
  return longestWord
...
function uniquifyArray(words) {
  ...
  // iterate over words
  for (let i = 0; i < words.length; i++) {
    // check if unique contains the word we are at
    // if not put it in there if yes don't
